Test_IAMReX are a custom set of python scripts that check the 代码完备性和运行结果results produced by the code.  We already , 涉及一些问题，涉及许多模块。同时，你也可以仿照example 来测试没有的。与此同时，借助github action，我们搭建了ci来自动检查运行这些测试。

Auto testing  or CI is supported by the github action.
